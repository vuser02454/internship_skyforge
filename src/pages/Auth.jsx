...
export default function Auth() {
  ...
  // Sync mode state when URL changes externally
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (mode === 'signup') {
      ...
      else setSuccessMsg("Check your email for the confirmation link to complete registration!");
    } else {
    ...
    <div className="bg-background text-on-background font-body-md min-h-screen flex flex-col md:flex-row overflow-x-hidden">
      ...
      <main className="w-full md:w-1/2 flex items-center justify-center p-gutter bg-surface">
        <div className="w-full max-w-[440px]">
          ...
          <div className="mb-stack-lg min-h-[60px]">
            <h2 className="font-headline-md text-headline-md text-primary mb-stack-sm">
              {mode === 'login' ? 'Welcome back' : 'Create an account'}
            </h2>
            <p className="text-on-surface-variant font-body-sm text-body-sm">
              {mode === 'login' ? 'Access your workspace and track your task progress.' : 'Join the platform and start finding opportunities.'}
            </p>
          </div>

          {/* Fluid Toggle Tabs */}
          <div className="relative flex bg-surface-container-low p-1 rounded-xl mb-stack-lg">
            {/* Sliding Highlight */}
            <div 
              className={`absolute top-1 bottom-1 w-[calc(50%-4px)] bg-surface rounded-lg shadow-[0px_4px_12px_rgba(46,49,146,0.05)] transition-all duration-300 ease-in-out ${mode === 'signup' ? 'left-[calc(50%)]' : 'left-1'}`}
            ></div>
            
            <button 
              onClick={() => handleToggle('login')}
              className={`relative z-10 flex-1 text-center py-2 text-label-caps font-label-caps rounded-lg transition-colors duration-300 ${mode === 'login' ? 'text-primary' : 'text-on-surface-variant hover:text-primary'}`}
            >
              Login
            </button>
            <button 
              onClick={() => handleToggle('signup')}
              className={`relative z-10 flex-1 text-center py-2 text-label-caps font-label-caps rounded-lg transition-colors duration-300 ${mode === 'signup' ? 'text-primary' : 'text-on-surface-variant hover:text-primary'}`}
            >
              Sign Up
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-stack-md">
            ...
            <div className={`transition-all duration-300 overflow-hidden ${mode === 'signup' ? 'max-h-40 opacity-100 mb-stack-lg' : 'max-h-0 opacity-0 mb-0'}`}>
